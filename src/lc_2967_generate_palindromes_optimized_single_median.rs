...
impl Solution {
    pub fn minimum_cost(mut nums: Vec<i32>) -> i64 {
        ...
        let median = ((nums[(n - 1) / 2] + nums[n / 2])) / 2;
        let target_idx = Self::binary_search(&palindromes, median);
        if target_idx == m {
            return cost(palindromes[m - 1]);
        } else if target_idx == 0 {
            return cost(palindromes[0]);
        } else {
...
mod test {
    ...
    #[test]
    fn lc_2967_generate_palindromes_optimized_single_median_1() {
        assert_eq!(Solution::minimum_cost(vec![1, 2, 3, 4, 5]), 6);
    }

    #[test]
    fn lc_2967_generate_palindromes_optimized_single_median_2() {
        assert_eq!(Solution::minimum_cost(vec![10, 12, 13, 14, 15]), 11);
    }

    #[test]
    fn lc_2967_generate_palindromes_optimized_single_median_3() {
